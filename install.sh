...
install_nginx() {
  ...
  echo -e "\n${CYAN}[*]${NC} Mengambil nginx.conf utama dari repo (gzip, real-ip Cloudflare, HSTS)..."
  echo -e "${YELLOW}[CATATAN]${NC} nginx.conf ini set_real_ip_from untuk range IP Cloudflare."
  echo -e "${YELLOW}[CATATAN]${NC} Kalau traffic ke server ini TIDAK lewat Cloudflare, X-Real-IP/"
  echo -e "${YELLOW}[CATATAN]${NC} access log bisa salah baca IP client. Edit /etc/nginx/nginx.conf"
  echo -e "${YELLOW}[CATATAN]${NC} kalau perlu."
  mkdir -p /etc/nginx
  NGINX_MAIN_CONF="/etc/nginx/nginx.conf"
  NGINX_MAIN_BACKUP="${NGINX_MAIN_CONF}.bak.$(date +%s)"
  [[ -f "$NGINX_MAIN_CONF" ]] && cp "$NGINX_MAIN_CONF" "$NGINX_MAIN_BACKUP"
  ...
  if wget -q --timeout=30 "$RAW/nginx/nginx.conf" -O "$NGINX_TMP" && [[ -s "$NGINX_TMP" ]]; then
    ...
    if nginx -t 2>/dev/null; then
      echo -e "${GREEN}[OK]${NC} nginx.conf utama terpasang & valid"
    else
      echo -e "${RED}[ERROR]${NC} nginx.conf baru bikin 'nginx -t' gagal, mengembalikan config asal..."
      if [[ -f "$NGINX_MAIN_BACKUP" ]]; then
        cp "$NGINX_MAIN_BACKUP" "$NGINX_MAIN_CONF"
      fi
      echo -e "${YELLOW}[WARN]${NC} Cek manual: nginx -t"
    fi
  else
    echo -e "${RED}[ERROR]${NC} Gagal download nginx.conf dari $RAW/nginx/nginx.conf"
    echo -e "${YELLOW}[WARN]${NC} Pakai nginx.conf default paket (kalau ada backup, tetap dipakai backup lama)."
    rm -f "$NGINX_TMP"
  fi
}
...
#   Port 80  : VMess nTLS | VLess nTLS | SlowDNS
#   Port 443 : VMess TLS | VLess TLS | Trojan TLS | SS TLS
#              VMess gRPC | VLess gRPC | Trojan gRPC | SS gRPC
#   NOTE: SSH-WS TIDAK lewat Nginx lagi. Client connect langsung ke
#         port ws-openssh / ws-dropbear (lihat addon/install-sshws.sh).
# ============================================================
...
server {
    ...
    # SSH-WS: sudah tidak lewat Nginx (langsung ke port ws-openssh/ws-dropbear)
...
install_script_files() {
  ...
  echo -ne "  Downloading addon/install-wstunnel.sh..."
  wget -q --timeout=30 "$RAW/addon/install-wstunnel.sh" -O "$SCRIPT_DIR/addon/install-wstunnel.sh"
  if [[ $? -ne 0 ]] || [[ ! -s "$SCRIPT_DIR/addon/install-wstunnel.sh" ]]; then
    echo -e " ${YELLOW}SKIP (belum ada di repo)${NC}"
    rm -f "$SCRIPT_DIR/addon/install-wstunnel.sh"
  else
    echo -e " ${GREEN}OK${NC}"
  fi

  echo -ne "  Downloading addon/install-haproxy.sh..."
  wget -q --timeout=30 "$RAW/addon/install-haproxy.sh" -O "$SCRIPT_DIR/addon/install-haproxy.sh"
  if [[ $? -ne 0 ]] || [[ ! -s "$SCRIPT_DIR/addon/install-haproxy.sh" ]]; then
    echo -e " ${YELLOW}SKIP (belum ada di repo)${NC}"
    rm -f "$SCRIPT_DIR/addon/install-haproxy.sh"
  else
    echo -e " ${GREEN}OK${NC}"
  fi
...
# ─── Firewall ──────────────────────────────────────────────
setup_firewall() {
  for port in 22 80 443 109 143 442 5300; do
...
main() {
  ...
  if [[ -f "$SCRIPT_DIR/addon/install-wstunnel.sh" ]]; then
    echo ""
    echo -e "${CYAN}══════════════════════════════════════════════════${NC}"
    echo -e "${WHITE}   MELANJUTKAN INSTALL ADDON: SSH-WS (wstunnel)   ${NC}"
    echo -e "${CYAN}══════════════════════════════════════════════════${NC}"
    bash "$SCRIPT_DIR/addon/install-wstunnel.sh"
  else
    echo -e "\n${YELLOW}[INFO]${NC} addon/install-wstunnel.sh tidak ditemukan di repo, dilewati."
  fi
  ...
  if [[ "$ADDON_INSTALLED" == "true" ]]; then
    ...
    echo -e "${GREEN}║${NC}  SSH-WS/SSL : openssh=$(systemctl is-active ws-openssh 2>/dev/null) dropbear=$(systemctl is-active ws-dropbear 2>/dev/null) stunnel=$(systemctl is-active ws-stunnel 2>/dev/null)"
  fi
