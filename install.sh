...
install_script_files() {
  ...
  # Download menu addon (SSH-WS/SSL, Service Status, Update) - opsional,
  # tidak menghentikan instalasi kalau belum ada di repo
  for f in sshws services update; do
    echo -ne "  Downloading menu/${f}.sh (addon)..."
    wget -q --timeout=30 "$RAW/menu/${f}.sh" -O "$SCRIPT_DIR/menu/${f}.sh"
    if [[ $? -ne 0 ]] || [[ ! -s "$SCRIPT_DIR/menu/${f}.sh" ]]; then
      echo -e " ${YELLOW}SKIP (belum ada di repo)${NC}"
      rm -f "$SCRIPT_DIR/menu/${f}.sh"
    else
      echo -e " ${GREEN}OK${NC}"
    fi
  done

  # Download installer addon SSH-WS/SSL - opsional
  ...
  # Download VERSION - opsional, dipakai fitur update
  wget -q --timeout=30 "$RAW/VERSION" -O "$SCRIPT_DIR/VERSION" 2>/dev/null
  [[ -s "$SCRIPT_DIR/VERSION" ]] || echo "1.0.0" > "$SCRIPT_DIR/VERSION"
...
main() {
  ...
  # ─── Install addon SSH-WS / SSH-WS-TLS / SSH-SSL sekaligus ───
  # (aditif — dijalankan setelah base VPN aktif, tidak mengganggu
  #  VMess/VLess/Nginx/Dropbear yang baru saja diinstall di atas)
  if [[ -f "$SCRIPT_DIR/addon/install-sshws.sh" ]]; then
    ...
    echo -e "${YELLOW}[INFO]${NC} Bisa diinstall belakangan lewat menu ${WHITE}vpn${NC} → [8]."
  ...
  if [[ "$ADDON_INSTALLED" == "true" ]]; then
    ...
    echo -e "${GREEN}║${NC}  SSH-WS     : $(systemctl is-active ws-proxy 2>/dev/null)"
    echo -e "${GREEN}║${NC}  SSH-SSL    : $(systemctl is-active ssh-connect-proxy 2>/dev/null)"
  fi
