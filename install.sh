...
# ─── Firewall ──────────────────────────────────────────────
setup_firewall() {
  for port in 22 80 443 109 143 442 400 465 777 900 8080; do
...
# ─── Install SSH WS & Stunnel ──────────────────────────────
install_ssh() {
  local domain="$1"
  echo -e "\n${CYAN}[*]${NC} Menginstall SSH WebSocket & SSL/TLS..."

  apt-get install -y -qq stunnel4 2>/dev/null
  mkdir -p /usr/local/bin /var/log/stunnel4 /var/run/stunnel4

  # ── ohpserver dari chanelog/bin ────────────────────────
  echo -e "  ${CYAN}[*]${NC} Mendownload ohpserver dari chanelog/bin..."
  wget -q "$REPO/ohpserver" -O /usr/local/bin/ohpserver
  if [[ $? -ne 0 ]] || [[ ! -s /usr/local/bin/ohpserver ]]; then
    echo -e "  ${YELLOW}[WARN]${NC} ohpserver gagal dari chanelog/bin, mencari alternatif..."
    # Fallback: cari dari GitHub releases
    wget -q "https://github.com/lfasmpao/open-http-puncher/releases/download/0.1/ohpserver-linux32"       -O /usr/local/bin/ohpserver 2>/dev/null
    if [[ ! -s /usr/local/bin/ohpserver ]]; then
      wget -q "https://github.com/lfasmpao/open-http-puncher/releases/latest/download/ohpserver-linux64"         -O /usr/local/bin/ohpserver 2>/dev/null
    fi
  fi

  if [[ -s /usr/local/bin/ohpserver ]]; then
    chmod +x /usr/local/bin/ohpserver
    echo -e "  ${GREEN}[OK]${NC} ohpserver terinstall"
  else
    echo -e "  ${YELLOW}[WARN]${NC} ohpserver tidak ditemukan, menggunakan python3 fallback..."
    apt-get install -y -qq python3 2>/dev/null
    cat > /usr/local/bin/ohpserver << 'PYEOF2'
#!/usr/bin/env python3
...
def listen(port):
    ...
    srv.listen(500)
    print(f"[OHP] Listening port {port} -> SSH 22")
    while True:
        c, _ = srv.accept()
        threading.Thread(target=handle, args=(c,), daemon=True).start()

import time
ports = sys.argv[1:] if len(sys.argv) > 1 else ["8080"]
for p in ports:
...
PYEOF2
  ...
  # ── Systemd service ohpserver ─────────────────────────
  # port 2082 = internal (nginx proxy → ssh-ws path)
  # port 8080 = langsung tanpa nginx
  cat > /etc/systemd/system/ssh-ws.service << 'EOF2'
[Unit]
Description=OHP SSH WebSocket Server
After=network.target

[Service]
Type=simple
ExecStart=/usr/local/bin/ohpserver 8080 2082
Restart=always
RestartSec=3
LimitNOFILE=65536
...
EOF2
  ...
  systemctl daemon-reload
  systemctl enable ssh-ws stunnel4 2>/dev/null
  systemctl restart ssh-ws 2>/dev/null
  systemctl restart stunnel4 2>/dev/null

  # Buat database SSH
  touch $SCRIPT_DIR/db/ssh.db

  echo -e "${GREEN}[OK]${NC} SSH WS (ohpserver) & SSL/TLS (stunnel) dikonfigurasi"
}
