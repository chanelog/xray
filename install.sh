...
install_ssh() {
  ...
  apt-get install -y -qq python3 stunnel4 screen 2>/dev/null

  mkdir -p /usr/local/bin /var/log/stunnel4 /var/run/stunnel4

  # SSH WS Proxy (python3) - listen port 2082, 8080
  cat > /usr/local/bin/ssh-ws << 'PYEOF2'
#!/usr/bin/env python3
...
def listen(port):
    ...
    try:
        ...
        srv.listen(200)
        print(f"[SSH-WS] Port {port} ready")
        while True:
            c, _ = srv.accept()
...
ports = [int(p) for p in sys.argv[1:]] if len(sys.argv) > 1 else [2082, 8080]
for p in ports:
...
PYEOF2
  ...
  # Systemd SSH WS (port 2082 internal + 8080 langsung)
...
EOF2
  ...
  systemctl restart ssh-ws stunnel4 2>/dev/null

  # Buat database SSH
  touch $SCRIPT_DIR/db/ssh.db

  echo -e "${GREEN}[OK]${NC} SSH WS & SSL/TLS dikonfigurasi"
}
