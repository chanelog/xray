...
install_script_files() {
  ...
  for f in vmess vless trojan ss nginx dropbear haproxy sysinfo changedomain uninstall sshws services update; do
...
main() {
  ...
  if [[ "$ADDON_INSTALLED" == "true" ]]; then
    ...
    echo -e "${GREEN}║${NC}  WS-Stunnel  : $(systemctl is-active ws-stunnel 2>/dev/null)"
  fi
