...
# ─── SSH WS Combined Status ─────────────────────────────────
# ON kalau salah satu dari service ws-ssh-tls / ws-ssh-ntls aktif
sshws_status() {
  if systemctl is-active --quiet ws-ssh-tls 2>/dev/null || systemctl is-active --quiet ws-ssh-ntls 2>/dev/null; then
    echo -e "${GREEN}● ON${NC}"
  else
    echo -e "${RED}● OFF${NC}"
  fi
