...
WS_PROXY_PORT=700          # port lokal ws-proxy (internal only, untuk SSH-WS)
CONNECT_PROXY_PORT=701     # port lokal ssh-connect-proxy (internal only, untuk SSH-SSL/stunnel)
...
MANAGED_SERVICES=(xray nginx dropbear stunnel4 ws-proxy ssh-connect-proxy)
...
service_display_name() {
  case "$1" in
    ...
    ws-proxy)          echo "SSH-WS Proxy" ;;
    ssh-connect-proxy) echo "SSH-SSL Connect Proxy" ;;
    *)                 echo "$1" ;;
  esac
}
