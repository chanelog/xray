...
# Port backend proxy SSH-WS (dipakai systemd ws-dropbear/ws-openssh/ws-stunnel
# DAN oleh konfigurasi Nginx/Stunnel4 di bawah — HARUS selalu sinkron)
WS_DROPBEAR_PORT=2095   # nginx /ssh-ws      -> ws-dropbear -> dropbear:109
WS_OPENSSH_PORT=2093    # nginx /ssh-ws-ssh  -> ws-openssh  -> sshd:22
WS_STUNNEL_PORT=700     # stunnel4 (127.0.0.1) -> ws-stunnel -> dropbear:143
SSH_BACKEND_PORT=143
STUNNEL_SSL_PORT=445
HAPROXY_SSL_PORT=446    # port terpisah dari stunnel4 (445) supaya tidak rebutan bind
...
#  SERVICE MANAGEMENT
# ════════════════════════════════════════════════════════════
MANAGED_SERVICES=(xray nginx dropbear stunnel4 ws-dropbear ws-openssh ws-stunnel haproxy)
...
service_display_name() {
  case "$1" in
    ...
    ws-stunnel)  echo "WS-Stunnel (backend Stunnel4)" ;;
    haproxy)     echo "HAProxy (SSH-WS-SSL alt, port $HAPROXY_SSL_PORT)" ;;
    *)           echo "$1" ;;
  esac
}
