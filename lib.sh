...
WS_OPENSSH_PORT=2093       # ws-openssh: SSH-WS langsung (client connect ke sini, bukan lewat Nginx)
WS_DROPBEAR_PORT=2095      # ws-dropbear: SSH-WS langsung (client connect ke sini, bukan lewat Nginx)
WS_STUNNEL_LOCAL_PORT=700  # ws-stunnel: hanya loopback, di belakang stunnel4
STUNNEL_SSL_PORT=445       # port publik SSH-SSL (stunnel4 -> ws-stunnel)
WSTUNNEL_PORT=8880         # proxy--ws (wstunnel/Rust): SSH-WS langsung, backend OpenSSH 127.0.0.1:22
NGINX_TLS_INTERNAL_PORT=8443  # dipakai HANYA kalau addon HAProxy terpasang: Nginx digeser dari
                               # 443 (publik) ke 127.0.0.1:8443 (loopback), HAProxy yang pegang 443
...
service_display_name() {
  case "$1" in
    ...
    ws-openssh)  echo "SSH-WS (OpenSSH, port $WS_OPENSSH_PORT)" ;;
    ws-dropbear) echo "SSH-WS (Dropbear, port $WS_DROPBEAR_PORT)" ;;
    ws-stunnel)  echo "SSH-SSL Backend (ws-stunnel)" ;;
    proxy--ws)   echo "SSH-WS (wstunnel/Rust, port $WSTUNNEL_PORT)" ;;
    haproxy)     echo "HAProxy (SNI router, port 443)" ;;
    *)         echo "$1" ;;
  esac
}
...
update_fetch_addon_bin() {
  ...
  tmp=$(mktemp)
  if wget -q --timeout=30 "$UPDATE_RAW/addon/install-wstunnel.sh" -O "$tmp" 2>/dev/null && [[ -s "$tmp" ]]; then
    mkdir -p "$SCRIPT_DIR/addon"
    cp "$tmp" "$SCRIPT_DIR/addon/install-wstunnel.sh"
    chmod +x "$SCRIPT_DIR/addon/install-wstunnel.sh"
  fi
  rm -f "$tmp"

  tmp=$(mktemp)
  if wget -q --timeout=30 "$UPDATE_RAW/addon/install-haproxy.sh" -O "$tmp" 2>/dev/null && [[ -s "$tmp" ]]; then
    mkdir -p "$SCRIPT_DIR/addon"
    cp "$tmp" "$SCRIPT_DIR/addon/install-haproxy.sh"
    chmod +x "$SCRIPT_DIR/addon/install-haproxy.sh"
  fi
  rm -f "$tmp"
