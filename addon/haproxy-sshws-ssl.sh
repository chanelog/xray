...
#  SSL Termination -> ws-stunnel -> Dropbear
#  Frontend: 0.0.0.0:${HAPROXY_SSL_PORT} (SSH-WS-SSL, alternatif dari Stunnel4)
#  Backend: 127.0.0.1:${WS_STUNNEL_PORT} (ws-stunnel)
#  CATATAN: port ini SENGAJA dibedakan dari Stunnel4 (port ${STUNNEL_SSL_PORT})
...
backend ws-stunnel-backend
  mode tcp
  balance roundrobin
  server ws-stunnel-1 127.0.0.1:${WS_STUNNEL_PORT} check inter 5s rise 2 fall 3
  timeout connect 60s
  timeout server 300s
...
echo -e "${GREEN}══════════════════════════════════════════════════${NC}"
echo -e "  ${YELLOW}HAProxy SSH-WS SSL${NC}   : port $HAPROXY_SSL_PORT (SSL)"
echo -e "  ${YELLOW}Backend${NC}             : ws-stunnel @ 127.0.0.1:$WS_STUNNEL_PORT"
