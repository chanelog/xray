...
# ─── 3b. Deploy ssh-connect-proxy (untuk SSH-SSL via stunnel) ──
# Banyak app client (mis. HTTP Custom tab "SSH"+SSL) mengirim baris
# "CONNECT host:port HTTP/1.0" lebih dulu sebelum data SSH mengalir.
# Dropbear tidak paham format ini, jadi butuh stub kecil yang selalu
# membalas "200 Connection Established" lalu meneruskan byte mentah.
echo -e "\n${CYAN}[*]${NC} Memasang ssh-connect-proxy (stub untuk SSH-SSL)..."

cat > /usr/local/bin/ssh-connect-proxy.py <<'PYEOF'
#!/usr/bin/env python3
"""
ssh-connect-proxy: stub HTTP CONNECT untuk client yang mengirim
"CONNECT host:port HTTP/1.0" (mis. HTTP Custom tab SSH+SSL) sebelum
data SSH mengalir. Selalu membalas 200 OK lalu relay byte mentah ke
server SSH lokal, mengabaikan host:port yang diminta client (karena
tujuan sebenarnya sudah pasti server SSH di VPS ini sendiri).
"""
import asyncio
import os

LISTEN_HOST = "127.0.0.1"
LISTEN_PORT = int(os.environ.get("CONNECT_PROXY_PORT", "701"))
BACKEND_HOST = "127.0.0.1"
BACKEND_PORT = int(os.environ.get("SSH_BACKEND_PORT", "143"))
...
async def handle_client(reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
    try:
        # Baca dan buang request line + header sampai baris kosong,
        # apa pun isinya (CONNECT/GET/dll).
        try:
            while True:
                line = await asyncio.wait_for(reader.readline(), timeout=10)
                if line in (b"\r\n", b""):
...
chmod +x /usr/local/bin/ssh-connect-proxy.py

cat > /etc/systemd/system/ssh-connect-proxy.service <<EOF2
[Unit]
Description=SSH-SSL Connect Proxy (HTTP CONNECT stub to Dropbear)
After=network.target dropbear.service

[Service]
Environment=CONNECT_PROXY_PORT=$CONNECT_PROXY_PORT
Environment=SSH_BACKEND_PORT=$SSH_BACKEND_PORT
ExecStart=/usr/bin/python3 /usr/local/bin/ssh-connect-proxy.py
Restart=on-failure
User=root
...
systemctl daemon-reload
systemctl enable ssh-connect-proxy 2>/dev/null
systemctl restart ssh-connect-proxy 2>/dev/null
echo -e "${GREEN}[OK]${NC} ssh-connect-proxy aktif di 127.0.0.1:$CONNECT_PROXY_PORT → 127.0.0.1:$SSH_BACKEND_PORT"
...
if grep -q "\[ssh-ssl\]" /etc/stunnel/stunnel.conf 2>/dev/null; then
  echo -e "${YELLOW}[UPDATE]${NC} Block [ssh-ssl] sudah ada, memperbarui agar lewat proxy stub..."
  # Hapus block [ssh-ssl] lama (dari baris "[ssh-ssl]" sampai sebelum
  # section berikutnya atau akhir file), lalu tulis ulang yang benar.
  awk '
    /^\[ssh-ssl\]/ { skip=1; next }
    /^\[/ && skip { skip=0 }
    !skip { print }
...
connect = 127.0.0.1:$CONNECT_PROXY_PORT
...
echo -e "${GREEN}══════════════════════════════════════════════════${NC}"
echo -e "  SSH-WS (nTLS) : port 80   path /ssh-ws        (via nginx + ws-proxy)"
echo -e "  SSH-WS (TLS)  : port 443  path /ssh-ws        (via nginx + ws-proxy)"
echo -e "  SSH-SSL       : port $STUNNEL_SSL_PORT (stunnel4 → ssh-connect-proxy → dropbear)"
echo -e "${GREEN}══════════════════════════════════════════════════${NC}"
