...
# ─── 2. Pastikan Python 3 tersedia ─────────────────────────
# ws-openssh / ws-dropbear / ws-stunnel sudah di-port ke Python 3 (sebelumnya
# pakai sintaks Python 2, yang sudah gak disediakan lagi di repo resmi
# Ubuntu 22.04+/Debian 11+ -- itu penyebab fitur ini dulu gagal start
# walau Xray tetap jalan). Python 3 sudah diinstall di langkah 1 di atas.
PYTHON_BIN=""
find_python3() {
...
echo -e "\n${CYAN}[*]${NC} Memeriksa ketersediaan Python 3..."
find_python3
...
if [[ -z "$PYTHON_BIN" ]]; then
  echo -e "${RED}[ERROR]${NC} Python 3 TIDAK ditemukan & gagal diinstall dari repo apt OS ini."
  echo -e "${RED}[ERROR]${NC} ws-openssh/ws-dropbear/ws-stunnel butuh Python 3, install dibatalkan."
  exit 1
else
  echo -e "${GREEN}[OK]${NC} Python 3 ditemukan di: $PYTHON_BIN"
fi
...
echo -e "${GREEN}[OK]${NC} Service unit dibuat (ExecStart pakai: $PYTHON_BIN)"
