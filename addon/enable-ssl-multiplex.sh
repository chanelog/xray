...
#   SNI lainnya (termasuk kosong / fronting acak) -> Stunnel4 -> Dropbear (raw SSH-SSL)
...
if [[ "$ok_domain" == "true" && "$ok_fallback" == "true" ]]; then
  echo -e "${GREEN}[OK]${NC} Kedua jalur (SNI domain -> Nginx, SNI lain -> Stunnel4) terverifikasi jalan"
else
  rollback "Verifikasi gagal (SNI domain -> Nginx: $ok_domain, SNI lain -> Stunnel4: $ok_fallback)"
fi
...
echo -e "${GREEN}══════════════════════════════════════════════════${NC}"
echo -e "  ${YELLOW}Xray + SSH-WS (payload)${NC} : $DOMAIN:443 (SNI harus = $DOMAIN persis)"
echo -e "  ${YELLOW}SSH-SSL (tanpa payload)${NC} : $DOMAIN:443 (SNI BEBAS/acak, langsung -> Dropbear)"
