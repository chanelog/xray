...
main_menu() {
  ...
  echo -e "${CYAN}$LINE${NC}"
  echo -e "  ${YELLOW}[8]${NC}  SSH / SSH-WS / SSH-SSL Management"
  echo -e "       ${DIM}SSH-WS, SSH-WS-TLS & SSH-SSL(Stunnel)${NC}"
  echo -e "${CYAN}$LINE${NC}"
  echo -e "  ${YELLOW}[9]${NC}  Status Layanan (ON/OFF)"
  echo -e "       ${DIM}Xray, Nginx, Dropbear, Stunnel4, SSH-WS Proxy${NC}"
  echo -e "${CYAN}$LINE${NC}"
  ...
  case "$choice" in
    ...
    8) bash $SCRIPT_DIR/menu/sshws.sh ;;
