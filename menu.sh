...
main_menu() {
  ...
  echo -e "       ${DIM}WS TLS & non-TLS Management${NC}"
  echo -e "${CYAN}$LINE${NC}"
  echo -e "  ${YELLOW}[3]${NC}  SSH WebSocket"
  echo -e "       ${DIM}SSH WS TLS & non-TLS Management${NC}"
  echo -e "${CYAN}$LINE${NC}"
  ...
  echo -e "       ${DIM}Update semua menu & fitur terbaru${NC}"
  echo -e "${CYAN}$LINE${NC}"
  ...
  case "$choice" in
    ...
    8) bash $SCRIPT_DIR/menu/update.sh ;;
