...
show_header() {
  ...
  echo -e "${CYAN}$LINE${NC}"
  echo -e "  Xray: $xray_st   Nginx: $nginx_st   Dropbear: $db_st   SSH-WS: $sshws_st"
  echo -e "${CYAN}$LINE${NC}"
  ...
  local ssh_count=$(wc -l < $SCRIPT_DIR/db/ssh.db 2>/dev/null || echo 0)
  echo -e "  ${YELLOW}Akun VMess${NC}: ${WHITE}$vmess_count${NC}   ${YELLOW}Akun VLess${NC}: ${WHITE}$vless_count${NC}   ${YELLOW}Akun SSH${NC}: ${WHITE}$ssh_count${NC}"
  echo -e "${CYAN}$LINE${NC}"
}
...
main_menu() {
  ...
  echo -e "${CYAN}$LINE${NC}"
  echo -e "  ${YELLOW}[1]${NC}  SSH Tunnel"
  echo -e "       ${DIM}SSH WS, SSL WS & SSL/TLS${NC}"
  echo -e "${CYAN}$LINE${NC}"
