...
show_header() {
  ...
  local proto_on proto_off
  systemctl is-active --quiet xray \
    && proto_on="${GREEN}ON${NC}" || proto_on="${RED}OFF${NC}"
  ...
  echo -e "${CYAN}$LINE${NC}"
  echo -e "  ${PURPLE}VMess WS TLS ${NC}(443) : $proto_on   ${PURPLE}VMess WS nTLS ${NC}(80) : $proto_on"
  echo -e "  ${PURPLE}VLess WS TLS ${NC}(443) : $proto_on   ${PURPLE}VLess WS nTLS ${NC}(80) : $proto_on"
