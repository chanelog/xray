...
sysinfo_menu() {
  ...
  local sshws_count=$(wc -l < "$SCRIPT_DIR/db/sshws.db" 2>/dev/null || echo 0)

  local xray_st nginx_st db_st sshws_tls_st sshws_ntls_st
  ...
  systemctl is-active --quiet ws-ssh-tls  && sshws_tls_st="${GREEN}● RUNNING${NC}"  || sshws_tls_st="${RED}● STOPPED${NC}"
  systemctl is-active --quiet ws-ssh-ntls && sshws_ntls_st="${GREEN}● RUNNING${NC}" || sshws_ntls_st="${RED}● STOPPED${NC}"
  ...
  echo -e "  ${YELLOW}SSH WS TLS ${NC}: $sshws_tls_st"
  echo -e "  ${YELLOW}SSH WS nTLS${NC}: $sshws_ntls_st"
  echo ""
  ...
  echo -e "  ${YELLOW}SSH WS     ${NC}: ${WHITE}$sshws_count akun${NC}"
  echo ""
