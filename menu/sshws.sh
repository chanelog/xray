...
ws_payload_string() {
  local domain="$1"
  # Payload mentah untuk app client (HTTP Injector / NPV Tunnel / HTTP Custom
  # mode Payload). [crlf] wajib ditulis literal, aplikasi yang mengganti
  # jadi \r\n saat mengirim.
...
do_create_ssh() {
  ...
  echo -e "${CYAN}$LINE${NC}"
  echo -e "  ${YELLOW}Payload WS (nTLS & TLS sama)${NC}:"
...
do_detail_ssh() {
  ...
  echo -e "      Payload:"
  echo -e "      ${YELLOW}$(ws_payload_string "$domain")${NC}"
  echo -e "${SLINE}"
