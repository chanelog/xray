...
sshws_header() {
  ...
  echo -e "  ${PURPLE}SSH-SSL       ${NC}: port ${WHITE}$STUNNEL_SSL_PORT${NC} (stunnel4 → dropbear, langsung/SNI-only)"
  echo -e "  ${PURPLE}SSH-SSL       ${NC}: port ${WHITE}${HAPROXY_SSL_PORT:-444}${NC} (haproxy → openssh, langsung/SNI-only — aktifkan di menu HAProxy)"
...
do_create_ssh() {
  ...
  echo -e "  ${WHITE}SSH-SSL     ${NC}: $domain  port $STUNNEL_SSL_PORT  (stunnel4 -> dropbear, SNI-only)"
  echo -e "  ${WHITE}SSH-SSL     ${NC}: $domain  port ${HAPROXY_SSL_PORT:-444}  (haproxy -> openssh, SNI-only)"
