...
sshws_header() {
  ...
  echo -e "  ${PURPLE}SSH-WS nTLS   ${NC}: port ${WHITE}8880${NC} path ${WHITE}/ssh-ws${NC}"
  echo -e "  ${PURPLE}SSH-WS nTLS   ${NC}: port ${WHITE}8080${NC} path ${WHITE}/ssh-ws${NC}"
  echo -e "  ${PURPLE}SSH-WS nTLS   ${NC}: port ${WHITE}2080${NC} path ${WHITE}/ssh-ws${NC}"
  echo -e "  ${PURPLE}SSH-WS nTLS   ${NC}: port ${WHITE}2082${NC} path ${WHITE}/ssh-ws${NC}"
...
do_create_ssh() {
  ...
  echo -e "  ${WHITE}SSH-WS nTLS ${NC}: $domain  port 8880 path /ssh-ws"
  echo -e "  ${WHITE}SSH-WS nTLS ${NC}: $domain  port 8080 path /ssh-ws"
  echo -e "  ${WHITE}SSH-WS nTLS ${NC}: $domain  port 2080 path /ssh-ws"
  echo -e "  ${WHITE}SSH-WS nTLS ${NC}: $domain  port 2082 path /ssh-ws"
  echo -e "  ${WHITE}SSH-WS TLS  ${NC}: $domain  port 443  path /ssh-ws"
  echo -e "${CYAN}$LINE${NC}"
  echo -e "  ${YELLOW}Payload WS (semua port nTLS sama)${NC}:"
...
do_detail_ssh() {
  ...
  echo -e "      Payload: ${YELLOW}$(ws_payload_string "$domain" "443")${NC}"
  echo -e "${CYAN}$LINE${NC}"
  echo ""
  echo -ne "  ${DIM}Tekan Enter untuk kembali...${NC}"; read -r
  sshws_menu
