...
# Fixed: Handle rate limit + reuse existing cert
#====================================================
...
# ============================================================
# SETUP SSL - Handle rate limit + reuse existing cert
# ============================================================
setup_ssl_acme() {
    echo -e "${YELLOW}[PROSES]${NC} Setting up SSL Certificate..."

    # Cek apakah cert sudah ada di acme.sh
    ACME_CERT_DIR="$HOME/.acme.sh/${DOMAIN}_ecc"
    ACME_CERT_DIR2="$HOME/.acme.sh/${DOMAIN}"

    if [[ -f "$ACME_CERT_DIR/${DOMAIN}.cer" ]] || [[ -f "$ACME_CERT_DIR2/${DOMAIN}.cer" ]]; then
        echo -e "${GREEN}[INFO]${NC} Cert sudah ada di acme.sh, menggunakan yang lama..."

        # Install cert yang sudah ada ke path standar
        mkdir -p /etc/letsencrypt/live/$DOMAIN
        "$HOME/.acme.sh/acme.sh" --install-cert -d $DOMAIN             --cert-file /etc/letsencrypt/live/$DOMAIN/cert.pem             --key-file /etc/letsencrypt/live/$DOMAIN/privkey.pem             --fullchain-file /etc/letsencrypt/live/$DOMAIN/fullchain.pem             --reloadcmd "systemctl reload nginx 2>/dev/null; systemctl restart xray 2>/dev/null"

        echo -e "${GREEN}[OK]${NC} SSL Certificate (existing) installed!"
        return 0
    fi

    # Cek apakah cert sudah ada di letsencrypt path
    if [[ -f "/etc/letsencrypt/live/$DOMAIN/fullchain.pem" ]]; then
        echo -e "${GREEN}[INFO]${NC} Cert sudah ada di /etc/letsencrypt/live/$DOMAIN/"
        echo -e "${YELLOW}[INFO]${NC} Menggunakan cert yang sudah ada..."
        return 0
    fi

    # Jika belum ada, generate baru
    echo -e "${YELLOW}[PROSES]${NC} Generate SSL Certificate baru dengan acme.sh..."

    # Stop nginx dulu biar port 80 free
    systemctl stop nginx 2>/dev/null || true

    # Coba generate dengan retry logic
    local retries=0
    local max_retries=3

    while [[ $retries -lt $max_retries ]]; do
        if "$HOME/.acme.sh/acme.sh" --issue -d $DOMAIN --standalone --force 2>/tmp/acme_error.log; then
            break
        else
            retries=$((retries + 1))
            if grep -q "rateLimited" /tmp/acme_error.log 2>/dev/null; then
                echo -e "${RED}[ERROR]${NC} Let's Encrypt rate limit terkena!"
                echo -e "${YELLOW}[INFO]${NC} Domain $DOMAIN sudah minta cert 5x dalam 7 hari."
                echo -e "${YELLOW}[INFO]${NC} Tunggu beberapa jam atau ganti domain."

                # Tanya user mau lanjut tanpa SSL atau ganti domain
                echo ""
                echo -e "${CYAN}[PILIHAN]${NC}"
                echo "1. Lanjut tanpa SSL (self-signed) - TIDAK RECOMMENDED"
                echo "2. Ganti domain"
                ...
                case $choice in
                    1)
                        echo -e "${YELLOW}[WARNING]${NC} Membuat self-signed certificate..."
                        mkdir -p /etc/letsencrypt/live/$DOMAIN
                        ...
                        echo -e "${YELLOW}[WARNING]${NC} Self-signed cert dibuat. Browser akan warning!"
                        return 0
                        ;;
                    2)
                        read -p "Masukkan domain baru: " new_domain
                        DOMAIN=$new_domain
                        echo "$DOMAIN" > /usr/local/etc/xray/domain.txt 2>/dev/null || true
                        retries=0
                        continue
                        ;;
                    3)
                        echo -e "${RED}[CANCEL]${NC} Installasi dibatalkan."
                        exit 1
                        ;;
                    *)
            ...
            if [[ $retries -lt $max_retries ]]; then
                echo -e "${YELLOW}[RETRY]${NC} Mencoba lagi ($retries/$max_retries)..."
                sleep 10
            fi
        fi
    done
    ...
    if [[ $retries -eq $max_retries ]]; then
        echo -e "${RED}[ERROR]${NC} Gagal generate SSL setelah $max_retries percobaan."
        exit 1
    fi
    ...
    echo -e "${GREEN}[OK]${NC} SSL Certificate berhasil dibuat!"
}
...
show_info() {
    ...
    echo -e "Dropbear SSL : ${GREEN}777${NC}"
    echo -e "Stunnel SSL  : ${GREEN}444${NC} (Dropbear) | ${GREEN}445${NC} (OpenSSH)"
